...
const elem={value:'',checked:true,style:{},innerHTML:'',addEventListener:()=>{}};
const ctx={console,Date,Math,TextEncoder,TextDecoder,_quiet:()=>{},_sanCard:x=>String(x||''),todayCards:()=> '2026-09-21',todayLocal:()=> '2026-09-21',proximaViradaTs:()=>Date.now()+864e5,localStorage:{get length(){return raw.size;},key:i=>[...raw.keys()][i]??null,getItem:k=>raw.get(k)||null,setItem:(k,v)=>raw.set(k,v),removeItem:k=>raw.delete(k)},window:{},document:{getElementById:()=>elem,querySelectorAll:()=>[]},$id:()=>elem,showToast:()=>{}};vm.createContext(ctx);
for(const [file,name] of [['11-db.js','DB'],['30-fsrs.js','FSRS'],['31-cards-config.js','CardsConfig'],['32-card-engine.js','CardEngine'],['44-anki-parity.js','AnkiParity'],['34-anki-export.js','AnkiExport'],['44-tela-cards.js','CardsScreen']])vm.runInContext(fs.readFileSync(new URL('src/js/'+file,root),'utf8')+'\nglobalThis.'+name+'='+name,ctx);
