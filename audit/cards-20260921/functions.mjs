...
D.saveCards([]);S._importParsed={kind:'json',cards:exported.cards,decks:[]};S.doImport();let imported=D.getCards();check('Backup round trip preserves 6000 contents',imported.length===6000&&imported.every((c,i)=>c.frente==='F'+i&&c.verso==='V'+i),{cards:imported.length});
