...
const AnkiProductParity = {
  ...
  _browserRows(){
    ...
    let rows=AnkiParity.notes().map(note=>{
      ...
      return {note,cards,nt,field:this._sortField(note,nt),tags:(note.tags||[]).slice(),flag:cards.reduce((m,c)=>Math.max(m,Number(c.flag)||0),0),suspended:cards.some(c=>c.suspenso),marked:cards.some(c=>c.favorito)};
    });
    const q=st.query.trim().toLowerCase();
    if(q)rows=rows.filter(r=>[r.field,r.nt&&r.nt.name,(r.tags||[]).join(' '),...Object.values(r.note.fields||{}).map(x=>this.plain(x))].join(' ').toLowerCase().includes(q));
  ...
  _syncBrowserBulk(rows){
    ...
    ['anki-browser-tags-btn','anki-browser-type-btn','anki-browser-suspend-btn','anki-browser-flag-btn','anki-browser-delete-btn'].forEach(id=>{const b=document.getElementById(id);if(b)b.disabled=!n;});
  },
  ...
  _bindChangeType(){
    document.getElementById('anki-change-type-save').addEventListener('click',()=>{
      ...
      for(const id of this._changeTypeIds||[]){
        ...
        if(target.kind==='cloze'&&!Object.values(fields).some(v=>/\{\{c\d+::/.test(String(v||'')))){showToast('Para mudar para Cloze, a nota precisa conter ao menos uma omissão {{c1::…}}.');return;}
  ...
  _bindNotetypes(){
    document.getElementById('anki-nt-add').addEventListener('click',()=>{
      UI.prompt([{key:'base',label:'Base',type:'select',value:'basic',options:[
        {value:'basic',label:'Básico'},{value:'basic_reversed',label:'Básico (e cartão invertido)'},{value:'basic_optional_reversed',label:'Básico (cartão invertido opcional)'},{value:'typing',label:'Básico (digite a resposta)'},{value:'cloze',label:'Omissão de palavras'}
  ...
  saveNotetypeEditor(){
    ...
    nt.fields=fieldRows.map((r,i)=>({name:names[i],ord:i,_source:r.dataset.source||''}));
    nt.templates=templateRows.map((r,i)=>({name:String(r.querySelector('.anki-template-name').value||('Card '+(i+1))).trim(),ord:i,qfmt:r.querySelector('.anki-qfmt').value,afmt:r.querySelector('.anki-afmt').value,_sourceOrd:r.dataset.sourceOrd}));
