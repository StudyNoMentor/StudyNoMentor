...
(() => {
  ...
  const R={
    ...
    painelConfigHtml(){
      ...
      return`<div class="rv8-panel" data-rv8-panel><div class="rv8-method-note"><b>Robusto · foco estatístico.</b><span>Escolhe até ${p.maxDisciplinas} disciplina(s), ordena seus tópicos e calcula a dose de questões. Seu método de resolução fica fora do algoritmo.</span></div><div class="pmc-grupo"><h5>1 · Recorte da prova</h5><p>Em que fase o motor raciocina e quantas frentes ele abre de uma vez.</p><div class="pmc-simple-grid"><label><span>Fase</span><select data-rv8="fase"><option value="auto" ${p.fase==='auto'?'selected':''}>Automática — segue o planejamento ativo</option><option value="pre" ${p.fase==='pre'?'selected':''}>Pré-edital</option><option value="pos" ${p.fase==='pos'?'selected':''}>Pós-edital</option></select><small>Hoje valendo: <b>${fase==='pre'?'Pré-edital':'Pós-edital'}</b>${p.fase==='auto'?' (derivada do planejamento)':' (fixada por você)'}. Pré dá peso 1 a todas as disciplinas; Pós usa os pesos manuais abaixo.</small></label><label><span>Disciplinas por vez</span><input data-rv8="maxDisciplinas" type="number" min="1" max="8" value="${p.maxDisciplinas}"><small>Quantas frentes distintas a força-tarefa abre. Era fixo em 3.</small></label></div></div><div class="pmc-grupo"><h5>2 · Régua e confiança</h5><p>O que conta como lacuna e quanta amostra o motor exige para acreditar nela.</p><div class="pmc-simple-grid"><label><span>Meta de domínio</span><div class="pmc-suffix"><input data-rv8="meta" type="number" min="60" max="99" value="${p.meta}"><em>%</em></div><small>Régua comum do TEC.</small></label><label><span>Amostra mínima confiável</span><input data-rv8="minAmostra" type="number" min="5" max="300" value="${p.minAmostra}"><small>Acima daqui a medição recebe confiança plena.</small></label><label><span>Mínimo para observar</span><input data-rv8="minObservacao" type="number" min="1" max="${p.minAmostra}" value="${p.minObservacao}"><small>Abaixo disso o tópico ainda é cedo demais até para disputar.</small></label><label><span>Incidência</span><select data-rv8="banca">${this._bancasHtml(p.banca)}</select><small>Fonte objetiva de relevância.</small></label></div><details class="pmc-avancado"><summary><b>Ajuste fino estatístico</b><small>Dois parâmetros da matemática interna. Os padrões funcionam — mexa só se souber o que muda.</small></summary><div class="pmc-simple-grid"><label><span>Força do prior</span><input data-rv8="forcaPrior" type="number" min="0.5" max="20" step="0.5" value="${p.forcaPrior}"><small>Quanto o motor desconfia de uma amostra pequena. Ele mistura a sua taxa observada com a meta antes de decidir, e este número diz o peso dessa desconfiança: alto, um tópico com 8 questões quase não se move; baixo, ele acredita no que viu. Regularização Beta, com padrão conservador.</small></label><label><span>Margem da lacuna</span><div class="pmc-suffix"><input data-rv8="margemLacunaPP" type="number" min="0" max="15" step="0.5" value="${p.margemLacunaPP}"><em>pp</em></div><small>A folga abaixo da meta que ainda NÃO é lacuna. Com meta 90% e margem 2pp, um tópico em 89% não entra na fila — evita perseguir diferenças menores que o próprio ruído da medição.</small></label></div></details></div><div class="pmc-grupo"><h5>3 · Dose de questões</h5><p>O tamanho do bloco que o motor recomenda para cada frente.</p><div class="pmc-simple-grid"><label><span>Dose base</span><input data-rv8="doseBase" type="number" min="5" max="100" value="${p.doseBase}"><small>Recalibrada conservadoramente por TEC + Extras.</small></label><label><span>Dose mínima</span><input data-rv8="doseMin" type="number" min="5" max="60" value="${p.doseMin}"><small>Piso do bloco. Nenhuma recomendação vem menor que isto, mesmo quando o motor calcula pouco.</small></label><label><span>Dose máxima</span><input data-rv8="doseMax" type="number" min="5" max="100" value="${p.doseMax}"><small>Teto do bloco. Impede que uma lacuna grande vire uma tarefa que não cabe na semana.</small></label></div></div>${rows}<footer class="rv8-footer"><span>Fontes: TEC + incidência + reforços em Extras.</span><b>Sem dependência de nomes/pesos/tempo do ciclo regular.</b><button type="button" class="btn-secondary pmc-reset" data-rv8-reset>↺ Restaurar padrões</button></footer></div>`;
    },
    /* `onChange` recebe a CHAVE do campo alterado. Sem isso quem escuta nao tem
     como saber se precisa remontar o painel (a `fase` muda quais campos
     existem) ou se basta atualizar o resumo — e remontar a cada tecla num
     campo numerico tira o foco de quem esta digitando. */
    bindConfig(host,onChange){if(!host)return;host.querySelectorAll('[data-rv8]').forEach(el=>el.addEventListener('change',()=>{const k=el.dataset.rv8;this.salvar({[k]:el.type==='number'?Number(el.value):el.value});if(onChange)onChange(k);}));const reset=host.querySelector('[data-rv8-reset]');if(reset)reset.addEventListener('click',async(ev)=>{const ok=typeof UI!=='undefined'&&UI.confirm?await UI.confirm('Voltar o Robusto aos valores de fábrica?\n\nOs ajustes deste modelo são descartados, inclusive o ajuste fino estatístico. O Simplificado não é afetado.',{title:'↺ Restaurar padrões',okText:'Restaurar'}):true;if(!ok)return;const antes=this.prefs();this.restaurar();const dep=this.prefs();const mudou=Object.keys(dep).filter(k=>String(antes[k])!==String(dep[k]));if(typeof showToast==='function')showToast(mudou.length?`Robusto restaurado ✓ — ${mudou.length} ajuste(s) voltaram ao padrão`:'O Robusto já estava nos padrões');if(ev&&ev.currentTarget&&ev.currentTarget.blur)ev.currentTarget.blur();if(onChange)onChange('__estrutura__');});const save=host.querySelector('[data-rv8-post-save]');if(save)save.addEventListener('click',()=>{const ds={};host.querySelectorAll('[data-rv8-post-active]').forEach(cb=>{const k=cb.dataset.rv8PostActive,w=host.querySelector(`[data-rv8-post-weight="${CSS.escape(k)}"]`),nome=w&&w.dataset.rv8PostName||k;ds[k]={nome,ativo:cb.checked,peso:Math.max(.1,n(w&&w.value,1))};});this.salvarPostConfig({configurado:true,disciplinas:ds,atualizadoEm:new Date().toISOString()});if(typeof showToast==='function')showToast('Pesos Pós-edital vinculados às disciplinas do TEC ✓');if(onChange)onChange('__estrutura__');});}
  };
