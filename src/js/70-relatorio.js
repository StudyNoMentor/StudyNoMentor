...
(function enhanceStudyReport(){
  ...
  StudyReport._advanced = function(entries,cfg){
    const a=this.aggregate(entries), days=Object.keys(a.byDay).sort();
    /* Planejamento atual: dias da pausa datada nao quebram a sequencia nem
       entram no denominador da frequencia — nao eram dias para estudar. */
    const pausa=(cfg&&cfg.scope==='plan'&&typeof PlanManager!=='undefined'&&PlanManager.pausedDaysBetween)?PlanManager:null;
    ...
    let maxStreak=0,current=0,prev=null; days.forEach(k=>{const d=new Date(k+'T00:00:00');let gap=prev?Math.round((d-prev)/86400000):0;if(prev&&pausa&&gap>1){const a1=new Date(prev);a1.setDate(a1.getDate()+1);const b1=new Date(d);b1.setDate(b1.getDate()-1);gap-=pausa.pausedDaysBetween(iso(a1),iso(b1));}if(prev&&gap===1)current++;else current=1;maxStreak=Math.max(maxStreak,current);prev=d;});
    ...
    const activeDays=days.length, avgSession=entries.length?a.minutes/entries.length:0, consistency=days.length&&days[0]&&days[days.length-1]?100*activeDays/Math.max(1,activeDays,Math.round((new Date(days[days.length-1]+'T00:00:00')-new Date(days[0]+'T00:00:00'))/86400000)+1-(pausa?pausa.pausedDaysBetween(days[0],days[days.length-1]):0)):0;
  ...
  StudyReport._mentorPages = function(cfg,entries){
    const d=this._advanced(entries,cfg), ins=this._insights(d), subs=d.a.bySub.slice().sort((x,y)=>y.minutes-x.minutes);
