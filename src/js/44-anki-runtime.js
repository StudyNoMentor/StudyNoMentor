...
const AnkiRuntime = {
  ...
  _darkTheme(){
    try{return typeof document!=='undefined'&&document.documentElement&&document.documentElement.getAttribute('data-theme')==='dark';}catch(_){return false;}
  },
  _syncThemeToFrames(){
  ...
  buildSrcdoc(nt, html, side, card, note, options) {
    ...
    const dark=this._darkTheme();
    // O iframe do template é uma origem isolada e não herda o tema do Study.
    // Anki usa a classe .nightMode para o modo noturno; propagamos a mesma classe.
    ...
    return '<!doctype html><html class="'+(dark?'nightMode':'')+'"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'+
      ...
      '<style>html,body{margin:0;padding:0;background:transparent;color:inherit}body{overflow-wrap:anywhere}.snm-anki-tts{cursor:pointer}.snm-anki-type-input{box-sizing:border-box;max-width:100%;padding:.4em .55em;font:inherit}#typeans{white-space:pre-wrap;font-family:monospace}.snm-anki-type-input{box-sizing:border-box;max-width:100%;padding:.4em .55em;font:inherit}.typeGood{color:#0a0}.typeBad{color:#c62828;text-decoration:line-through}.typeMissed{color:#c62828;text-decoration:underline}</style><style>'+css+'</style><style id="snm-anki-theme">'+themeCss+'</style>'+math+
      '</head><body class="card '+this._escAttr(side||'question')+nightClass+'">'+body+bootstrap+'</body></html>';
  },
