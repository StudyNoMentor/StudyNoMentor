...
const RelationalStore = {
  enabled: true,
  // Revlog volumoso usa RAM tipada + mutações SQL incrementais. O fallback
  // antigo continua disponível para testes/ambientes sem o store relacional.
  fastRevlog: true,
  _applying: false,
  ...
  _clearProfileMemory(profileId, opts) {
    ...
    try { if (typeof DB !== 'undefined' && DB._clearRevlogMemoryPrefix) DB._clearRevlogMemoryPrefix(p); } catch (e) { _quiet(e, 'revlog-cache-clear'); }
    this._applying = true;
    try {
  ...
  _clearSuffixMemory(profileId, suffix) {
    const p = this._pfx(profileId), rx = new RegExp('^p:[^:]+:' + suffix + '
    this._applying = true;
    try {
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
  ...
  async _refreshCorePartial(profileId, specs) {
    ...
    specs.forEach((s, i) => {
      ...
      this._group(loaded[i]).forEach((arr, plan) => {
        const key = pfx + 'p:' + plan + ':' + a.suffix;
        const mapped = arr.map(a.map);
        if (a.suffix === 'revlog' && typeof DB !== 'undefined' && DB._setRevlogMemory) DB._setRevlogMemory(key, mapped);
        else this._memSet(key, JSON.stringify(mapped));
      });
    });
  },
  _rpcMissing(error) {
  ...
  _applyCoreBundle(profileId, d, opts) {
    ...
    const putGroups=(rows,suffix,map)=>{
      this._group(rows).forEach((arr,plan)=>{
        const key=pfx+'p:'+plan+':'+suffix, mapped=arr.map(map);
        if(suffix==='revlog' && typeof DB!=='undefined' && DB._setRevlogMemory) DB._setRevlogMemory(key,mapped);
        else this._memSet(key,JSON.stringify(mapped));
      });
    };
    const appliers=this._coreGroupAppliers();
  ...
  _revlogRow(profileId, planId, x, fallbackPosition) {
    x=x||{};
    return {
      ...
      position:Number(x._position)||Number(fallbackPosition)||1,
  ...
  queueRevlogAppend(key, row) {
    ...
    if(!p || p.scope!=='plan' || p.sub!=='revlog') return false;
  ...
  queueRevlogDelete(key, row) {
    if(this._applying || !this.isReady()) return false;
    const p=this._keyParts(key);
    if(!p || p.scope!=='plan' || p.sub!=='revlog' || !row) return false;
  ...
  queueRevlogReplace(key, rows) {
    ...
    this._queue('revlog:replace',()=>{
      const mapped=arr.map((x,i)=>this._revlogRow(p.profileId,p.planId,x,i+1));
      return this._replacePlanRows('study_review_log',p.profileId,p.planId,mapped,'review_pk');
    });
    return true;
  },
...
window.RelationalStore=RelationalStore;
    ...
    if (suffix === 'revlog') {
      try { if (typeof DB !== 'undefined' && DB._clearRevlogMemoryPrefix) DB._clearRevlogMemoryPrefix(p); } catch (e) { _quiet(e, 'revlog-cache-clear-suffix'); }
    }
    this._applying = true;
    try {
      for (let i = 0; i < localStorage.length; i++) {
