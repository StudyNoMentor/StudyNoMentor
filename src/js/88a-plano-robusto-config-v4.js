...
(() => {
  ...
  const C={
    ...
    _sanear(m,raw){
      ...
      r.minCiclosIntervencao=Math.round(clamp(r.minCiclosIntervencao,3,30));r.confiancaMin=clamp(r.confiancaMin,.30,.95);r.ganhoBaixaResposta=clamp(r.ganhoBaixaResposta,0,10);r.taxaTeoria=clamp(r.taxaTeoria,30,90);r.questoesBloco=Math.round(clamp(r.questoesBloco,5,50));
