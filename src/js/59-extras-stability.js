...
(() => {
  ...
  /* Uma única pilha para qualquer modal de Extras. O mais novo sempre fica acima
     e os anteriores ficam inertes, evitando foco/clique "atrás" da janela ativa. */
  const OverlayStack = {
    base: 200000,
    seq: 0,
    observer: null,
    register(el) {
      ...
      if (!el.dataset.extrasLayer) el.dataset.extrasLayer = String(++this.seq);
      el.style.zIndex = String(this.base + Number(el.dataset.extrasLayer) * 20);
      this.sync();
    },
    list() {
      if (typeof document === 'undefined') return [];
      return [...document.querySelectorAll(OVERLAY_SELECTOR)]
        .filter(x => x.isConnected !== false)
        .sort((a,b) => Number(a.dataset.extrasLayer || 0) - Number(b.dataset.extrasLayer || 0));
    },
    sync() {
      if (typeof document === 'undefined') return;
      const list = this.list(), top = list[list.length - 1] || null;
      list.forEach(el => {
        const active = el === top;
      ...
      document.documentElement.classList.toggle('extras-modal-open', !!list.length);
    },
    install() {
      if (typeof document === 'undefined') return;
      ...
      this.observer = new MutationObserver((records) => {
        ...
        records.forEach(r => {
          [...(r.addedNodes || [])].forEach(n => {
            ...
            if (n.matches && n.matches(OVERLAY_SELECTOR)) { this.register(n); changed = true; }
            if (n.querySelectorAll) n.querySelectorAll(OVERLAY_SELECTOR).forEach(x => { this.register(x); changed = true; });
          });
          if ((r.removedNodes || []).length) changed = true;
        });
      ...
      document.addEventListener('keydown', (e) => {
        ...
        if (!top) return;
        const btn = top.querySelector('.ra-x,.rg-x,.xsc-close,[data-cancel],[data-rg-cancel]');
