...
(() => {
  ...
  const uid=prefix=>{
    try{if(typeof crypto!=='undefined'&&typeof crypto.randomUUID==='function')return prefix+'-'+crypto.randomUUID();}catch(_){/* local fallback */}
  ...
  const A={
    ...
    _save(z){
      ...
      try{const raw=JSON.stringify(z);if(DB.setRaw)DB.setRaw(this._key(),raw);else localStorage.setItem(this._key(),raw);}catch(e){if(typeof _quiet==='function')_quiet(e,'robusto-audit-save');}
      return z;
    },
    ...
    _configSnapshot(){const modo=this._perfilAtaque();let prefs={};try{prefs=RC&&RC.prefs?clone(RC.prefs(modo)):{};}catch(_){prefs={};}return{modo,revisaoAuditoria:R.REVISAO_AUDITORIA||null,versaoMotor:R.VERSAO||null,prefs};},
    _ensureConfig(z){const snap=this._configSnapshot(),id='cfg-'+hash(snap);if(!z.configs[id])z.configs[id]=snap;return id;},
    _scope(){
      const out={};
      try{const D=window.DesempenhoTecScreen||((typeof DesempenhoTecScreen!=='undefined')?DesempenhoTecScreen:null);if(D){out.modo=D.scopeMode||null;out.retratosSelecionados=D.selectedSnapIds&&typeof D.selectedSnapIds.size==='number'?D.selectedSnapIds.size:null;out.dataInicio=D.rangeStart||null;out.dataFim=D.rangeEnd||null;}}catch(_){/* opcional */}
    ...
    _cand(c,rank){
      ...
      return{rank,disciplina:c.disciplina||'',topico:c.nome||'',score:round(c.score),taxa:round(c.taxa,2),qJanela:Math.max(0,Math.round(n(c.qJanela,c.item&&c.item.qJanela))),qHist:Math.max(0,Math.round(n(c.qHist,c.item&&c.item.qHist))),alvoGlobal:Math.max(0,Math.round(n(c.alvo))),doseDiaria:c.doseDiaria==null?null:Math.max(0,Math.round(n(c.doseDiaria))),meta:c.meta==null?null:round(c.meta,2),minAmostra:c.minAmostra==null?null:Math.round(n(c.minAmostra)),banca:c.banca||null,fase:c.fase||null,componentes:numericMap(c.componentes),dominio:pick(dom,['nivel','rotulo','n','taxa','metaOperacional','metaCompetitiva','metaElite','pOperacional','pCompetitiva','pElite','diasDesdeMedicao','vencido','validadeDias']),calibracao:pick(cal,['nivel','n','nTopico','nDisciplina','nGlobal','ganho100','qPorPonto','confianca','evidenciaIntervencao','baixaResposta','limiarCiclos']),tempo:pick(tempo,['confiavel','segundosPorQuestao','q','lancamentos','escopo']),intervencao:this._intervencao(c),otimizacao:this._otimizacao(c),politica:c.politica?pick(c.politica,['n','nElegiveis','aprendida','shrink','validacao','fase','perfil','outcome','alpha','aplicado','regularizacao']):null,auditoria:c.auditoria?pick(c.auditoria,['robustoV6','revisaoAuditoria','posteriorPriorReal','aprendizadoSegregado','scoreCanonicoNoOtimizador']):null};
    },
    ...
    registrarCalculo(res,ctx){
      if(!res||res.erro)return null;this.reconciliar();const z=this._load(),configId=this._ensureConfig(z),modoInterface=(ctx&&ctx.modoInterface)||null,sig=this._decisionSignature(res,configId,modoInterface),ult=[...z.events].reverse().find(e=>e.tipo==='decisao');
    ...
    _decisionFor(z,extra){const s=extra&&extra.origemPlano&&extra.origemPlano.sugestao||{},d=norm(extra&&extra.disciplina),t=norm(extra&&extra.origemPlano&&extra.origemPlano.topico);return [...z.events].reverse().find(e=>e.tipo==='decisao'&&(!s.criadoEm||e.ts.slice(0,10)>=String(s.criadoEm).slice(0,10))&&(e.selecionados||[]).some(c=>norm(c.disciplina)===d&&norm(c.topico)===t))||null;},
    _creationPayload(z,extra){
      const s=extra&&extra.origemPlano&&extra.origemPlano.sugestao||{},dec=this._lastDecisionId?z.events.find(e=>e.id===this._lastDecisionId):this._decisionFor(z,extra);
      return{extraId:String(extra.id||''),decisionId:dec&&dec.id||null,configId:dec&&dec.configId||null,modoInterface:s.modoInterface||null,fase:s.fase||null,perfilAtaque:s.configRobusto&&s.configRobusto.modo||s.perfilAtaque||null,disciplina:extra.disciplina||extra.origemPlano.disciplina||'',topico:extra.origemPlano.topico||'',alvoGlobal:Math.max(0,Math.round(n(s.alvoGlobal,extra.alvo))),doseDiaria:s.doseDiaria==null?null:Math.max(0,Math.round(n(s.doseDiaria))),score:round(s.score),componentes:numericMap(s.componentes),intervencao:s.intervencao?clone(s.intervencao):null,otimizacao:s.otimizacaoV6||s.otimizacaoV5||null,criadoEm:s.criadoEm||extra.createdAt||null};
    },
    _registrarCriacaoEm(z,extra){
      const s=extra&&extra.origemPlano&&extra.origemPlano.sugestao||{};if(!/^robusto-v/.test(String(s.motor||'')))return null;const eid=String(extra.id||''),ja=z.events.find(e=>e.tipo==='atividade_criada'&&e.extraId===eid);if(ja)return ja;return this._append(z,'atividade_criada',this._creationPayload(z,extra));
    },
    ...
    reconciliar(){
      const z=this._load();let alterou=false,extras=[];try{extras=DB.getExtras?DB.getExtras():[];}catch(_){extras=[];}
      for(const e of extras){const s=e&&e.origemPlano&&e.origemPlano.sugestao||{};if(!/^robusto-v/.test(String(s.motor||'')))continue;if(!z.events.some(x=>x.tipo==='atividade_criada'&&x.extraId===String(e.id||''))){this._registrarCriacaoEm(z,e);alterou=true;}const r=this._resultado(e),fp=hash(r),id=String(e.id||'');if(z.resultadoFingerprints[id]===fp)continue;z.resultadoFingerprints[id]=fp;this._append(z,'resultado_snapshot',Object.assign({fingerprint:fp},r));alterou=true;}
      if(alterou)this._save(z);return{alterou,eventos:z.events.length};
    },
