...
(function initCiclo() {
  ...
  let importedCycleSource = null; // cópia transitória: nunca cria vínculo com o planejamento de origem

  function cloneCycleData(v) {
    return v == null ? v : JSON.parse(JSON.stringify(v));
  }

  // Escolhe a melhor referência disponível de outro planejamento. O ciclo ativo
  // é preferido; em seguida vem a memória do último ciclo montado e, para dados
  // legados, a última semana fechada do histórico.
  function cycleSuggestionForPlan(planId) {
    if (!planId) return null;
    const current = DB.getCurrentCycleForPlan ? DB.getCurrentCycleForPlan(planId) : null;
    if (current && Array.isArray(current.subjects) && current.subjects.length) {
      return {
        kind: 'ciclo atual',
        weeklyHours: current.weeklyHours,
        mode: current.mode,
        subjects: cloneCycleData(current.subjects)
      };
    }
    const last = DB.getLastCycleSetupForPlan ? DB.getLastCycleSetupForPlan(planId) : null;
    if (last && Array.isArray(last.subjects) && last.subjects.length) {
      return {
        kind: 'último ciclo montado',
        weeklyHours: last.weeklyHours,
        mode: last.mode,
        subjects: cloneCycleData(last.subjects)
      };
    }
    const hist = DB.getCycleHistoryForPlan ? DB.getCycleHistoryForPlan(planId) : [];
    const latest = Array.isArray(hist) && hist.length ? hist[hist.length - 1] : null;
    if (latest && Array.isArray(latest.subjects) && latest.subjects.length) {
      return {
        kind: 'última semana fechada',
        weeklyHours: latest.weeklyHours,
        mode: latest.mode,
        subjects: cloneCycleData(latest.subjects)
      };
    }
    return null;
  }

  function defaultCycleImportStatus() {
    return 'Copia carga horária, matérias e tempos como ponto de partida. Datas, histórico e grade não são copiados, e o planejamento de origem não é alterado.';
  }

  function refreshCycleImportPlans() {
    const wrap = document.getElementById('cycle-import-wrap');
    const sel = document.getElementById('cycle-import-plan');
    const btn = document.getElementById('btn-import-cycle');
    const status = document.getElementById('cycle-import-status');
    if (!wrap || !sel || !btn) return;

    const activeId = PlanManager.getActivePlanId();
    const options = (PlanManager.getPlans() || [])
      .filter(p => p && p.id !== activeId)
      .map(p => ({ plan: p, source: cycleSuggestionForPlan(p.id) }))
      .filter(x => x.source)
      .sort((a, b) => String(a.plan.nome || '').localeCompare(String(b.plan.nome || ''), 'pt-BR', { sensitivity: 'base' }));

    wrap.style.display = options.length ? 'block' : 'none';
    if (!options.length) {
      sel.innerHTML = '';
      btn.disabled = true;
      if (status) status.textContent = defaultCycleImportStatus();
      return;
    }
  ...
  on('cycle-import-plan', 'change', () => {
    const sel = document.getElementById('cycle-import-plan');
    const btn = document.getElementById('btn-import-cycle');
    if (btn) btn.disabled = !(sel && sel.value);
  });

  on('btn-import-cycle', 'click', () => {
    const sel = document.getElementById('cycle-import-plan');
    const status = document.getElementById('cycle-import-status');
    const planId = sel && sel.value;
    if (!planId) return;

    const source = cycleSuggestionForPlan(planId);
    const plan = (PlanManager.getPlans() || []).find(p => p.id === planId);
    if (!source || !plan) {
      showToast('Não encontrei um ciclo reutilizável nesse planejamento.');
      refreshCycleImportPlans();
      return;
    }

    // Importar é iniciar um NOVO rascunho a partir da origem. Isso evita que os
    // tempos do ciclo que estava sendo editado no destino sobrescrevam a cópia.
    pendingCycle = null;
    const definedByName = {};
    (source.subjects || []).forEach(s => {
      if (!s || !s.nome) return;
      const v = s.definidoMin != null ? s.definidoMin : s.sugeridoMin;
      if (v != null) definedByName[CycleEngine.normKey(s.nome)] = Number(v) || 0;
    });
    importedCycleSource = {
      planId,
      planName: plan.nome,
    ...
    writeWeeklyHours(source.weeklyHours || 20);
    renderSetupSubjects(source.subjects || []);

    const activeKeys = new Set(DB.getActiveSubjects().map(s => CycleEngine.normKey(s.nome)));
    const novos = (source.subjects || []).filter(s => s && s.nome && !activeKeys.has(CycleEngine.normKey(s.nome))).length;
    ...
    showToast('Ciclo carregado como sugestão. Revise antes de confirmar.');
  });
  ...
  on('btn-calc-cycle', 'click', () => {
    ...
    // 2) tempos trazidos de outro planejamento: são ponto de partida e
    // sobrescrevem a memória local, mas continuam editáveis na revisão.
    if (importedCycleSource && importedCycleSource.definedByName) {
      Object.keys(importedCycleSource.definedByName).forEach(k => {
        prevDefined[k] = importedCycleSource.definedByName[k];
      });
    }
    // 3) rascunho em edição (tem prioridade máxima)
