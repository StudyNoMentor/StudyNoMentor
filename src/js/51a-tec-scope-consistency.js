  ...
  PE.calcular = function calcularComExecucaoReal() {
    ...
    const decorar = (x, motivo) => {
      ...
      if (motivo !== 'diagnostico' && x.status && x.taxa != null && x.taxa < n(p.metaDominio, 85)) {
        const critico = x.taxa < n(p.faixaCritico, 50);
        const fragil = !critico && x.taxa < n(p.faixaFragil, 65);
        x.status.ordem = critico
          ? `Teoria primeiro; depois uma sessão de até ${e.sessao} questões. Ciclo: ${e.ciclo} questões.`
          : `Faça até ${e.sessao} questões nesta sessão. Ciclo: ${e.ciclo} questões, alternando as matérias.`;
        x.status.acao = critico
          ? `Retome a teoria deste assunto antes de insistir em volume. Depois faça blocos de até ${e.sessao} questões, revise os erros e alterne para outra disciplina. O ciclo fecha com ${e.ciclo} novas questões e então você reavalia; não precisa perseguir de uma vez nem a amostra estatística${e.qMedirAlvo ? ` de ${e.qMedirAlvo}` : ''} nem o custo estimado${e.custoEstimadoQ ? ` de ${e.custoEstimadoQ}` : ''}.`
          : `${fragil ? 'Vá pelo caminho do erro: faça o bloco, revise só o que errou e siga para outra disciplina.' : 'Use o bloco para confirmar os pontos frágeis e revise apenas os erros.'} Sessões de até ${e.sessao} questões; ciclo curto de ${e.ciclo}. ${e.qMedirAlvo ? `A amostra de ${e.qMedirAlvo} questões é uma referência estatística TOTAL, construída ao longo de vários ciclos — não uma tarefa para hoje.` : ''}`;
      }
      return x;
    };
    ...
    if (typeof PC.avaliar === 'function') {
      ...
      PC.avaliar = function avaliarComMigracaoSegura(extra, ref) {
        ...
        try {
          ...
          if (v && extra && extra.status !== 'concluida' && o && !o.modeloExecucao && (v.feito || 0) === 0) {
  ...
  if (EX) {
    ...
    const depoisDeRender = () => {
      try {
        ...
        const itens = PC && typeof PC.emCurso === 'function' && EX._planoRefOperacional ? PC.emCurso(EX._planoRefOperacional()) : [];
        if (ritmo && itens && itens.length) {
          const ds = new Set(itens.map(v => String((v.origem && v.origem.disciplina) || 'Sem disciplina'))).size;
          ritmo.innerHTML = `<b>rodízio ${ds} mat.</b><span> · ≤${MAX_SESSAO}q/sessão</span>`;
        }
