...
(() => {
  ...
  const I=window.PlanoSugestoesInfraV2;if(!I||typeof DB==='undefined')return;
  ...
  const DEFAULTS=Object.freeze({meta:90,minAmostra:20,minObservacao:5,banca:'__todas__',forcaPrior:3,margemLacunaPP:3,doseMin:10,doseBase:15,doseMax:30,minCiclosDose:4,shrinkDose:8,ganhoMinPP100q:3,minQuestoesTempoExtras:30});
  const KEY='robusto-tec-v8',PREF_KEYS=Object.keys(DEFAULTS);

  const R={
    VERSAO:8,REVISAO_AUDITORIA:8,MOTOR:'robusto-v8',KEY,DEFAULTS,PREF_KEYS,
    prefs(){
    ...
    _snapshotSeries(snapshot){
      ...
      try{const D=window.DesempenhoTecScreen||((typeof DesempenhoTecScreen!=='undefined')?DesempenhoTecScreen:null);if(D&&D.selectedSnapIds&&D.selectedSnapIds.size){const ids=D.selectedSnapIds,z=snaps.filter(s=>ids.has(s.id));if(z.length)snaps=z;}}catch(e){if(typeof _quiet==='function')_quiet(e,'robusto-v8-scope-series');}
    ...
    _incRows(p){let rows=[];try{rows=I.incidencia?I.incidencia()||[]:[];}catch(e){if(typeof _quiet==='function')_quiet(e,'robusto-v8-incidencia');}const b=norm(p.banca);return rows.filter(r=>p.banca==='__todas__'||!p.banca||norm(r&&r.banca)===b);},
    ...
    _componentes(pool,snapshot,p,extraObs){
      const incRows=this._incRows(p),incIndex=this._incIndex(incRows),histIndex=this._historicoIndex(snapshot,pool,p),RX=window.ReforcoTecExtrasV8,rxCfg={minCiclosDose:p.minCiclosDose,ganhoMinPP100q:p.ganhoMinPP100q,_observacoes:extraObs};
      for(const c of pool){
    ...
    calcular(){
      ...
      const RX=window.ReforcoTecExtrasV8,extraObs=RX&&RX.observacoes?RX.observacoes():[];pool=this._componentes(pool,snapshot,p,extraObs);const grupos=this._disciplinas(pool,fase);if(!grupos.length)return{erro:'sem-candidatos',modo:'robusto',fase,itens:[],arquitetura:this.arquitetura()};
      const itens=[],rxCfg={...p,_observacoes:extraObs};for(const g of grupos.slice(0,3)){
