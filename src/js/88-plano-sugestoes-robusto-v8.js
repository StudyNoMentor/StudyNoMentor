...
(() => {
  ...
  const R={
    ...
    _componentes(pool,snapshot,p){
      const incRows=this._incRows(p),RX=window.ReforcoTecExtrasV8;
      for(const c of pool){
        const post=this.posterior(c.taxa,c.qJanela,p.meta,p),histTec=this._historicoTec(c,snapshot,p),inc=this._incidencia(c,incRows),histExtra=RX?RX.historico(c.disciplina,c.nome,{minCiclosDose:p.minCiclosDose,ganhoMinPP100q:p.ganhoMinPP100q}):{n:0,nEfetivo:0};
    ...
    _disciplinas(pool,fase){
      ...
      for(const g of map.values()){
        ...
        if(fase==='pos'){const pc=cfg[norm(g.disciplina)];if(!pc||pc.ativo!==true)continue;g.peso=Math.max(.1,n(pc.peso,1));}else g.peso=1;
      ...
      if(fase==='pos'){const mx=Math.max(1,...active.map(x=>x.peso));active.forEach(g=>{g.pesoNorm=g.peso/mx;g.scoreDisciplina=.72*g.scoreBase+.28*g.pesoNorm*100;});}
    ...
    calcular(){
      ...
      const itens=[],RX=window.ReforcoTecExtrasV8;
      for(const g of grupos.slice(0,3)){
        const top=g.topicos[0],rx=RX?RX.prescrever({...top,minAmostra:p.minAmostra,meta:p.meta,componentes:{lacuna:top._gap,evidencia:top._evid,persistencia:top._persist}},p):{dose:p.doseBase,fonte:'tec',tempo:{confiavel:false,fonte:'indisponivel',minutosEstimados:null}},dose=Math.max(1,Math.round(n(rx.dose,p.doseBase))),tempo=rx.tempo||{confiavel:false,fonte:'indisponivel',minutosEstimados:null};
        itens.push({...top,modo:'robusto',fase,score:g.scoreDisciplina,scoreDisciplina:g.scoreDisciplina,scoreTopico:top.scoreTopico,alvo:dose,quantidadeRecomendada:dose,meta:p.meta,minAmostra:p.minAmostra,banca:p.banca,pesoPost:fase==='pos'?g.peso:1,prescricao:rx,
          ...
          motivo:`Disciplina #${itens.length+1}: prioridade ${g.scoreDisciplina.toFixed(0)}/100 · tópico líder ${top.scoreTopico.toFixed(0)}/100${fase==='pos'?` · peso manual ${g.peso}`:''}.`,
