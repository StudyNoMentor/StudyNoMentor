...
const SectionSync = {
  ...
  _savePend(id, listaOverride) {
    try {
      const lista = listaOverride ? [...new Set(listaOverride)] : [...this._dirty];
  ...
  restorePending(id) {
    ...
    this._loadPend(id).forEach(s => this._ensureDirty(s));
  ...
  async flushBeforeRead(id, opts) {
    ...
    pend.forEach(s => this._ensureDirty(s));
    this._savePend(id);
  ...
  async _syncManifest(id, revs) {
    ...
    for (const del of delEntries) {
      ...
      try {
        ...
        restantes.push(del);
        console.warn('[SectionSync] exclusão remota falhou:', del.section, e);
      }
    }
