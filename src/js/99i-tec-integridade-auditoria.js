...
(() => {
  ...
  if (L) {
    ...
    L.priorityBreakdown=function(topic,st) {
      ...
      try { usage=this.usageByDiscipline(st||this.state()).get(norm(topic.disciplina))||usage; }
      catch (e) { if (typeof _quiet==='function') _quiet(e,'lacunas-priority-usage'); }
    ...
    if (!box) {
      ...
      box.addEventListener('click',async e=>{
        ...
        if (b.dataset.trustAction==='audit') {
          ...
          try { await navigator.clipboard.writeText(text); if (typeof showToast==='function') showToast('Auditoria de resoluções copiada ✓'); }
          catch (err) { if (typeof _quiet==='function') _quiet(err,'tec-integrity-copy-audit'); if (typeof showToast==='function') showToast('Não foi possível copiar a auditoria neste navegador.'); }
        }
      });
    }
