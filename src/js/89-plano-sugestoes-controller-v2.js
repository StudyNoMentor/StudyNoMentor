...
(() => {
  ...
  const C={
    ...
    _renderModal(screen,p,res){
      ...
      body.querySelectorAll('[data-ps-modo]').forEach(b=>b.addEventListener('click',()=>{p=this.salvar({modo:b.dataset.psModo});screen._rv4Open=false;this._recalcularModal(screen,p);}));
      ...
      const tg=body.querySelector('[data-rv4-toggle]'),host=body.querySelector('[data-rv4-host]');
      if(tg&&host){const setOpen=v=>{screen._rv4Open=!!v;host.hidden=!v;tg.setAttribute('aria-expanded',v?'true':'false');};setOpen(!!screen._rv4Open);tg.addEventListener('click',()=>setOpen(host.hidden));if(R.bindConfig)R.bindConfig(host,()=>{screen._rv4Open=true;this._recalcularModal(screen,p);});}
    },
    ...
    criar(screen,p,res){
      ...
      itens.forEach(c=>{
        if(!c||!c.item)return;const x=c.item,iv=c.intervencaoV4||c.intervencao,passos=iv&&iv.passos&&iv.passos.length?' Sequência: '+iv.passos.map(s=>s.tipo+(s.quantidade?' '+s.quantidade+'q':s.minutos?' '+s.minutos+'min':'')).join(' → ')+'.':'';
        const e=DB.addExtra({titulo:PlanoCiclo.titulo(c.nome,'reforco',x.membros),tipo:'questoes',disciplina:c.disciplina||'',unidade:'questoes',alvo:Math.max(1,c.alvo),periodo:'unica',contaMetricas:false,obs:`Gerado pelo Plano · ${c.modo==='robusto'?'Robusto':'Simplificado'} ${c.fase==='pos'?'Pós':'Pré'}.${iv&&iv.rotulo?' Intervenção: '+iv.rotulo+'.':''}${passos}`});
        if(!e)return;
        const origem=PlanoCiclo.origem(c.nome,c.disciplina,Object.assign({},x,{custoQ:c.alvo,taxa:c.taxa??x.taxa}),{motivo:'reforco'});
        origem.sugestao={versao:this.VERSAO,motor:c.modo==='robusto'?R.MOTOR:S.MOTOR,modoInterface:p.modo,fase:c.fase,meta:c.meta,minAmostra:c.minAmostra,banca:c.banca||null,score:Math.round(num(c.score)*10)/10,alvoGlobal:Math.max(1,c.alvo),doseDiaria:c.doseDiaria||null,componentes:c.componentes||{},politica:c.politica||null,configPlano:c.configPlano||null,configRobusto:c.configRobusto||null,intervencao:iv||null,otimizacaoV4:c.otimizacaoV4||null,auditoria:c.auditoria||null,arquitetura:c.modo==='robusto'?R.arquitetura():S.arquitetura(),criadoEm:typeof todayLocal==='function'?todayLocal():new Date().toISOString().slice(0,10)};
