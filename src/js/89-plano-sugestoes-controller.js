/* ============================================================================
   PUXAR DO PLANO — ORQUESTRADOR NEUTRO V5
   ----------------------------------------------------------------------------
   Conhece as APIs públicas do Simplificado e do Robusto V8, mas nunca mistura
...
(() => {
  ...
  const C={
    ...
    robusto(){const r=R.calcular();try{if(window.PlanoRobustoAuditV2)PlanoRobustoAuditV2.registrarCalculo(r,{modoInterface:'robusto'});}catch(e){if(typeof _quiet==='function')_quiet(e,'plano-controller-v5-audit');}return r;},
    comparar(){
      let s,r;try{s=this.simplificado();}catch(e){s={erro:'falha-simplificado',modo:'simplificado',itens:[]};if(typeof _quiet==='function')_quiet(e,'plano-controller-v5-simple');}
      try{r=R.calcular();if(window.PlanoRobustoAuditV2)PlanoRobustoAuditV2.registrarCalculo(r,{modoInterface:'comparar'});}catch(e){r={erro:'falha-robusto',modo:'robusto',itens:[]};if(typeof _quiet==='function')_quiet(e,'plano-controller-v5-robusto');}
    ...
    _recalcularModal(screen,p){const host=document.getElementById('pl-lista');if(host)host.innerHTML='<div class="ps-loading">Recalculando sugestões…</div>';setTimeout(()=>{try{const res=this.calcular(p);screen._psResultado=res;this._renderModal(screen,p,res);}catch(e){if(typeof _quiet==='function')_quiet(e,'plano-controller-v5-recalc');}},0);},
    ...
    criar(screen,p,res){
      ...
      itens.forEach(c=>{
        ...
        const e=DB.addExtra({titulo:PlanoCiclo.titulo(c.nome,'reforco',x.membros),tipo:'questoes',disciplina:c.disciplina||'',unidade:'questoes',alvo:q,periodo:'unica',contaMetricas:false,obs:`Gerado pelo Plano · ${rob?'Robusto V8':'Simplificado'} ${c.fase==='pos'?'Pós':'Pré'} · ${q} questões aprofundadas.`});
