...
(() => {
  ...
  const C={
    ...
    robusto(){const r=R.calcular();try{if(window.PlanoRobustoAudit)PlanoRobustoAudit.registrarCalculo(r,{modoInterface:'robusto'});}catch(e){if(typeof _quiet==='function')_quiet(e,'plano-controller-v5-audit');}return r;},
    comparar(){
      ...
      try{r=R.calcular();if(window.PlanoRobustoAudit)PlanoRobustoAudit.registrarCalculo(r,{modoInterface:'comparar'});}catch(e){r={erro:'falha-robusto',modo:'robusto',itens:[]};if(typeof _quiet==='function')_quiet(e,'plano-controller-v5-robusto');}
    ...
    criar(screen,p,res){
      ...
      itens.forEach(c=>{
        ...
        if(rob){try{if(window.PlanoRobustoAudit)PlanoRobustoAudit.registrarCriacao(DB.getExtras().find(z=>z.id===e.id)||e);}catch(err){if(typeof _quiet==='function')_quiet(err,'plano-controller-v5-audit-create');}}
        total++;
      });
