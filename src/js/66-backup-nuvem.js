...
const CloudBackup = {
  ...
  async moverAncora(id) {
    ...
    const marcar = async (linhaId, valor) => {
      const { error } = await CloudStore.client.from(this.TABLE).update({ ancora: valor }).eq('id', linhaId);
      if (error) throw error;
    };
    try {
      ...
      if (!this._avisouAncora) {
        ...
        console.info('[CloudBackup] a âncora não pôde ser movida (falta a permissão de UPDATE na coluna `ancora` — ver BANCO-DE-DADOS.md). A âncora atual continua protegida.');
      }
