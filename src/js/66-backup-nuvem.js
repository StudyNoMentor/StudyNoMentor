/* ═══════════════════ BACKUP NO BANCO DE DADOS ══════════════════════════════
   O PostgreSQL relacional guarda o estado atual do perfil em tabelas `study_*`.
   Isso resolve a persistência operacional, mas não substitui histórico: um erro
   lógico confirmado no banco pode sobrescrever o estado corrente corretamente.

   `profile_backups` é a camada separada de recuperação. Ela guarda fotos
   imutáveis e comprimidas do perfil, fora das tabelas operacionais, para que um
   estado anterior possa ser restaurado em qualquer aparelho.

   Política:
     · uma âncora permanente por perfil;
     · foto diária, confirmada pelo próprio banco;
     · foto antes de operações de risco;
     · retenção em faixas (dias, semanas e meses);
...
/* ── A TELA (Configurações → Dados) ────────────────────────────────────────
   Lista as fotos persistidas no banco. Elas não dependem deste aparelho. */
const CloudBackupUI = {
  _quando(iso) {
  ...
  async render() {
    ...
    if (!CloudBackup.enabled) {
      host.innerHTML = '<p class="hint">⚠️ A tabela <code>profile_backups</code> não está disponível. O estado atual continua no banco relacional, mas as fotos históricas automáticas ficam indisponíveis até a tabela ser restaurada. A exportação manual em .json continua disponível.</p>';
      return;
    }
    ...
    const aviso = CloudBackup._ultimoErro
      ? `<p class="hint" style="color:var(--warn-text);background:var(--warn-soft);border-radius:10px;padding:8px 12px;margin:0 0 10px;">⚠️ O último envio ao banco não deu certo: ${escapeHtml(CloudBackup._ultimoErro)}. As fotos já confirmadas abaixo continuam válidas e a exportação manual em .json segue disponível.</p>`
      : '';
    if (!linhas.length) {
  ...
  _ligar(host) {
    ...
    host.querySelectorAll('.cbk-restore').forEach(b => b.addEventListener('click', async () => {
      ...
      const ok = await UI.confirm(
        'Restaurar esta cópia sobre o perfil atual?\n\nAntes de aplicar, o app tenta guardar no banco uma nova foto do estado atual. Depois da restauração, o resultado é confirmado nas tabelas relacionais e o app recarrega.',
