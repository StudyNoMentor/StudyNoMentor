...
(() => {
  ...
  /* ── RECONCILIAÇÃO RELACIONAL ───────────────────────────────────────────
     A camada local-first foi aposentada. A UI continua podendo mostrar um aviso
     discreto de sincronização, mas o dado canônico sempre vem do PostgreSQL. */
  ...
  async function reconcileProfile(id) {
    if (!id || !window.CloudStore || !CloudStore.isReady || !CloudStore.isReady() ||
        !CloudStore.isLoggedIn || !CloudStore.isLoggedIn() || !window.RelationalStore) return false;
    if (window.ProfileManager && ProfileManager.getActiveProfileId &&
        ProfileManager.getActiveProfileId() !== id) return false;
    StartupTrace.mark('reconciliacao-relacional-inicio');
    syncNote('Consultando o banco…');
    try {
      await RelationalStore.flush();
      await RelationalStore.catchUp(id, 'ux-reconcile');
      StartupTrace.mark('reconciliacao-relacional-ok');
      return true;
    } catch (e) {
      quiet(e, 'reconciliacao-relacional');
      StartupTrace.mark('reconciliacao-relacional-adiada', { erro: String(e && e.message || e || '') });
      return false;
    } finally {
  ...
  function scheduleReconcile(id) {
    if (!id) return;
    setTimeout(() => reconcileProfile(id), 120);
  }

  function installLocalFirst() {
    /* Compatibilidade nominal para diagnósticos antigos.
       Não envolve nem substitui ProfileUI.enterProfile. */
    return false;
  }
