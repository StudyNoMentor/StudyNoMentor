...
(() => {
  ...
  const PlanFastCache = {
    _origCalc: (typeof PlanoEngine !== 'undefined' && PlanoEngine.calcular) ? PlanoEngine.calcular : null,
    _porSnapshot: new WeakMap(),
    _hits: 0,
    _misses: 0,
    _depth: 0,
    _restaurarCalc: null,
    ...
    _stable(v, vistos) {
      ...
      vistos.add(v);
      if (Array.isArray(v)) return '[' + v.map(x => this._stable(x, vistos)).join(',') + ']';
      return '{' + Object.keys(v).sort().map(k => JSON.stringify(k) + ':' + this._stable(v[k], vistos)).join(',') + '}';
    },
    _contexto() {
      try { return DB._profilePrefix() + '|' + DB._activePlanId(); }
      catch (_) { return 'default'; }
    },
    calcular(snap, prefs) {
      if (!this._origCalc || !snap || (typeof snap !== 'object' && typeof snap !== 'function')) {
        return this._origCalc ? this._origCalc.call(PlanoEngine, snap, prefs) : null;
      }
      let mapa = this._porSnapshot.get(snap);
      if (!mapa) { mapa = new Map(); this._porSnapshot.set(snap, mapa); }
      const key = this._contexto() + '|' + this._stable(prefs || {});
      if (mapa.has(key)) { this._hits++; return mapa.get(key); }
      const r = this._origCalc.call(PlanoEngine, snap, prefs);
      this._misses++;
      if (mapa.size >= 16) mapa.delete(mapa.keys().next().value);
      mapa.set(key, r);
      return r;
    },
    atual(patch) {
      if (typeof DesempenhoTecScreen === 'undefined' || typeof PlanoEngine === 'undefined') return null;
      const snap = DesempenhoTecScreen.scopedSnapshot && DesempenhoTecScreen.scopedSnapshot();
      if (!snap) return null;
      const prefs = Object.assign({}, PlanoEngine.prefs ? PlanoEngine.prefs() : {}, patch || {});
      return this.calcular(snap, prefs);
    },
    withCache(fn) {
      if (!this._origCalc || typeof PlanoEngine === 'undefined') return fn();
      if (this._depth++ === 0) {
        this._restaurarCalc = PlanoEngine.calcular;
        PlanoEngine.calcular = (snap, prefs) => this.calcular(snap, prefs);
      }
      try { return fn(); }
      finally {
        if (--this._depth === 0) {
          PlanoEngine.calcular = this._restaurarCalc || this._origCalc;
          this._restaurarCalc = null;
        }
      }
    },
    invalidate() { this._porSnapshot = new WeakMap(); },
    stats() { return { hits: this._hits, misses: this._misses }; }
  };
  if (typeof window !== 'undefined') window.PlanFastCache = PlanFastCache;

  const invalidarApos = (obj, nome) => {
    if (!obj || typeof obj[nome] !== 'function' || obj[nome]._planFastWrapped) return;
    const original = obj[nome];
    const wrapped = function () {
      const r = original.apply(this, arguments);
      PlanFastCache.invalidate();
      return r;
    };
    wrapped._planFastWrapped = true;
    obj[nome] = wrapped;
  };
  if (typeof DB !== 'undefined') {
    ['saveIncidencia', 'saveTecSnapshots'].forEach(n => invalidarApos(DB, n));
  }
  if (typeof PlanoEngine !== 'undefined') invalidarApos(PlanoEngine, 'salvarPrefs');

  if (typeof ReforcoFila !== 'undefined') {
