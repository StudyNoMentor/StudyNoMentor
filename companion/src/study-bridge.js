...
(() => {
  ...
  function enqueuePlus(requestId,payload) {
    if (!requestId) return;
    const message={ kind:'plus-ai-request', requestId:String(requestId), payload:payload||{} };
    try {
  ...
  function ackPlus(requestId) {
    ...
    try {
      ...
      else chrome.runtime.sendMessage({ kind:'plus-ai-ack', requestId:String(requestId) },()=>{ const ignored=chrome.runtime.lastError; void ignored; });
    } catch (_) {}
  }
  function connect() {
    if (port) return;
  ...
  window.addEventListener('pageshow',(event)=>{
    ...
    if (!port) connect();
    setTimeout(()=>{ if (pageReady || plusProxyReady) { announce(); flushBuffer(); resync(); } },50);
  });
  document.addEventListener('visibilitychange',()=>{
    lifecycle={ type:'visibilitychange', at:new Date().toISOString(), persisted:false, visibility:document.visibilityState };
    if (document.visibilityState==='visible') { if (!port) connect(); setTimeout(()=>{ if (pageReady || plusProxyReady) resync(); },50); }
  });
