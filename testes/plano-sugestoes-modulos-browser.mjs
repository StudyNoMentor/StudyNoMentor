...
try{
  ...
  await page.waitForFunction(()=>typeof switchScreen==='function'&&typeof PlanoSugestoesV2==='object'&&typeof PlanoSugestoesSimplificadoV2==='object'&&typeof PlanoSugestoesRobustoV2==='object'&&typeof PlanoEngine==='object'&&typeof ExtrasScreen==='object',{timeout:30000});
  await page.evaluate(()=>{
  ...
  const criadas=await page.evaluate(()=>DB.getExtras().slice(-3).map(e=>({disc:e.disciplina,alvo:e.alvo,status:e.status,motor:e.origemPlano?.sugestao?.motor,modo:e.origemPlano?.sugestao?.modoInterface,dose:e.origemPlano?.sugestao?.doseDiaria,alvoMeta:e.origemPlano?.sugestao?.alvoGlobal,arquitetura:e.origemPlano?.sugestao?.arquitetura})));
  ...
  assert.ok(criadas.every(x=>/^(simplificado-v2|robusto-v3)$/.test(x.motor)&&x.modo==='comparar'),'a origem deve registrar o motor efetivo independente e o modo Comparar');
  ...
  console.log('OK: Puxar do Plano V2 — motores independentes, Comparar por ranking e execução única 3×1.');
} finally {
  await browser.close();
