...
try{
  ...
  await page.evaluate(()=>PlanoMotoresGovernanca.salvar({robusto:false,simplificado:false}));await page.evaluate(()=>DesempenhoTecScreen.render());assert.equal(await tab.isVisible(),true,'sem motores o Plano legado deve voltar');assert.equal(await page.locator('[data-tpm-entry]').count(),0,'sem motores não deve existir seletor de fonte');
