...
try{
  ...
  const boot=await page.evaluate(()=>({fonte:typeof TecPlanoFonteMotorV2,robusto:typeof PlanoSugestoesRobustoV8,controller:typeof PlanoSugestoesV5,gov:typeof PlanoMotoresGovernancaV5,tec:typeof DesempenhoTecScreen}));for(const[k,v]of Object.entries(boot))assert.equal(v,'object',`bootstrap ausente ${k}: ${JSON.stringify(boot)}`);
  await page.evaluate(()=>{
    ...
    PlanoMotoresGovernancaV5.restaurar();PlanoSugestoesRobustoV8.restaurar();TecPlanoFonteMotorV2.salvar('robusto');
  });
  // O seletor deve existir já na tela de abertura do Desempenho TEC, antes de entrar em Plano.
  ...
  await page.evaluate(()=>PlanoMotoresGovernancaV5.salvar({robusto:false,simplificado:false}));await page.evaluate(()=>DesempenhoTecScreen.render());assert.equal(await tab.isVisible(),false,'sem motores o Plano deve ficar oculto');assert.equal(await page.locator('[data-tpm-entry]').count(),0,'sem motores não deve existir seletor de fonte');
  await page.evaluate(()=>{PlanoMotoresGovernancaV5.restaurar();TecPlanoFonteMotorV2.salvar('robusto');DesempenhoTecScreen.render();DesempenhoTecScreen.switchTecTab('plano');});
