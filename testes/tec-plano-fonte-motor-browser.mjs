...
try{
  ...
  const boot=await page.evaluate(()=>({fonte:typeof TecPlanoFonteMotorV2,robusto:typeof PlanoSugestoesRobustoV8,controller:typeof PlanoSugestoesV5,gov:typeof PlanoMotoresGovernancaV6,tec:typeof DesempenhoTecScreen}));for(const[k,v]of Object.entries(boot))assert.equal(v,'object',`bootstrap ausente ${k}: ${JSON.stringify(boot)}`);
  await page.evaluate(()=>{
    try{ProfileUI.hideGate();}catch(e){if(typeof _quiet==='function')_quiet(e,'tpm-v8-gate');}
    ...
    PlanoMotoresGovernancaV6.restaurar();PlanoSugestoesRobustoV8.restaurar();TecPlanoFonteMotorV2.salvar('robusto');
  });
  ...
  await page.locator('[data-tpm-entry] [data-tpm-source="simplificado"]').click();await page.waitForFunction(()=>TecPlanoFonteMotorV2.fonte()==='simplificado');assert.equal(await page.evaluate(()=>PlanoSugestoesV5.prefs().modo),'simplificado');
  await page.locator('[data-tpm-entry] [data-tpm-source="robusto"]').click();await page.waitForFunction(()=>TecPlanoFonteMotorV2.fonte()==='robusto');assert.equal(await page.evaluate(()=>PlanoSugestoesV5.prefs().modo),'robusto');
  ...
  await page.locator('[data-tpm-selector] [data-tpm-source="simplificado"]').click();await page.waitForSelector('[data-tpm-output][data-tpm-model="simplificado"]');assert.equal(await page.evaluate(()=>TecPlanoFonteMotorV2.fonte()),'simplificado');const txtS=await page.locator('[data-tpm-output]').textContent();assert.match(txtS,/O Simplificado não modela tempo/i);assert.doesNotMatch(txtS,/min\/questão/i,'Simplificado não pode emprestar relógio do Robusto');cards=page.locator('[data-tpm-output] [data-tpm-rec]');const sdiscs=await cards.evaluateAll(xs=>xs.map(x=>x.dataset.disciplina));assert.equal(new Set(sdiscs).size,sdiscs.length);

  const tab=page.locator('.tec-subtab[data-tectab="plano"]');
  await page.evaluate(()=>PlanoMotoresGovernancaV6.salvar({robusto:false,simplificado:true}));await page.evaluate(()=>{DesempenhoTecScreen.render();DesempenhoTecScreen.switchTecTab('plano');});assert.equal(await tab.isVisible(),true,'Plano deve continuar disponível com apenas Simplificado');assert.equal(await page.locator('[data-tpm-source="robusto"]').count(),0);assert.equal(await page.locator('[data-tpm-output]').getAttribute('data-tpm-model'),'simplificado');
  await page.evaluate(()=>PlanoMotoresGovernancaV6.salvar({robusto:true,simplificado:false}));await page.evaluate(()=>{DesempenhoTecScreen.render();DesempenhoTecScreen.switchTecTab('plano');});assert.equal(await tab.isVisible(),true,'Plano deve continuar disponível com apenas Robusto');assert.equal(await page.locator('[data-tpm-source="simplificado"]').count(),0);assert.equal(await page.locator('[data-tpm-output]').getAttribute('data-tpm-model'),'robusto');
  await page.evaluate(()=>PlanoMotoresGovernancaV6.salvar({robusto:false,simplificado:false}));await page.evaluate(()=>DesempenhoTecScreen.render());assert.equal(await tab.isVisible(),false,'sem motores o Plano deve ficar oculto');assert.equal(await page.locator('[data-tpm-entry]').count(),0,'sem motores não deve existir seletor de fonte');
  await page.evaluate(()=>{PlanoMotoresGovernancaV6.restaurar();TecPlanoFonteMotorV2.salvar('robusto');DesempenhoTecScreen.render();DesempenhoTecScreen.switchTecTab('plano');});
  await page.setViewportSize({width:360,height:640});assert.ok(await overflow('#tec-panel-plano')<=4,'seletor/recomendação deve caber em 360px');assert.ok(await overflow('[data-tpm-entry]')<=4,'seletor de abertura deve caber em 360px');assert.deepEqual(errors,[],`erros no navegador: ${errors.join(' | ')}`);
  console.log('OK: Desempenho TEC V8 — fonte explícita desde a abertura, Plano recalculado e tempo não inventado.');
