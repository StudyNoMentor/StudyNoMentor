...
try{
  ...
  const search=page.locator('[data-rv5-search-input]');await search.fill('meta competitiva');assert.equal(await page.locator('[data-rv5-row]:visible').count(),1);await search.fill('');await page.locator('[data-rv5-only-mod]').check();assert.ok(await page.locator('[data-rv5-row]:visible').count()>=1);await page.locator('[data-rv5-only-mod]').uncheck();
