...
try{
  ...
  const boot=await page.evaluate(()=>({switchScreen:typeof switchScreen,central:typeof PlanoMotoresCentralTecV2,governanca:typeof PlanoMotoresGovernancaV5,controller:typeof PlanoSugestoesV3,simplificado:typeof PlanoSugestoesSimplificadoV2,robusto:typeof PlanoSugestoesRobustoV5,configRobusto:typeof PlanoRobustoConfigV5,audit:typeof PlanoRobustoAuditV1,tec:typeof DesempenhoTecScreen}));
  ...
  assert.equal(await page.locator('[data-pmc-robust-audit]').count(),1,'Robusto deve expor auditoria humana local');assert.equal(await page.locator('[data-pmc-audit-export]').count(),1);assert.equal(await page.locator('[data-pmc-audit-refresh]').count(),1);assert.match(await page.locator('[data-pmc-robust-audit]').textContent(),/Log real do Robusto|local-only/i);
  const audit0=await page.evaluate(()=>PlanoRobustoAuditV1.gerarPayload());assert.equal(audit0.schema,'studynomentor.robusto-audit.v1');assert.match(audit0.meta.privacidade,/local-only/);assert.ok(Array.isArray(audit0.calibrationDataset));assert.ok((await overflow('#tec-panel-motores'))<=4);
  ...
  await page.locator('[data-pmc-audit-refresh]').click();await page.waitForTimeout(80);assert.equal(await page.locator('[data-pmc-robust-audit]').count(),1);
  ...
  const auditAfterPull=await page.evaluate(()=>PlanoRobustoAuditV1.gerarPayload().diagnostico);assert.ok(auditAfterPull.decisoes>=1,'abrir o Robusto deve deixar decisão auditável');

  await page.evaluate(()=>PlanoMotoresGovernancaV5.salvar({robusto:false,simplificado:true}));await page.evaluate(()=>{switchScreen('desempenhotec');DesempenhoTecScreen.render();DesempenhoTecScreen.switchTecTab('motores');});assert.equal(await tab.isVisible(),true);assert.equal(await page.locator('[data-pmc-engine="simplificado"]').count(),1);assert.equal(await page.locator('[data-pmc-engine="robusto"]').count(),0);assert.equal(await page.locator('[data-pmc-robust-audit]').count(),0);
  await page.evaluate(()=>PlanoMotoresGovernancaV5.salvar({robusto:true,simplificado:false}));await page.evaluate(()=>{DesempenhoTecScreen.render();DesempenhoTecScreen.switchTecTab('motores');});assert.equal(await page.locator('[data-pmc-engine="simplificado"]').count(),0);assert.equal(await page.locator('[data-pmc-engine="robusto"]').count(),1);assert.equal(await page.locator('[data-pmc-robust-audit]').count(),1);
  ...
  await page.setViewportSize({width:360,height:640});await page.evaluate(()=>{switchScreen('desempenhotec');DesempenhoTecScreen.render();DesempenhoTecScreen.switchTecTab('motores');});assert.ok((await overflow('#tec-panel-motores'))<=4,'central V6 com auditoria deve caber em 360px');assert.ok((await overflow('[data-pmc-robust-audit]'))<=4,'painel de auditoria deve caber em 360px');
  assert.deepEqual(errors,[],`erros no navegador: ${errors.join(' | ')}`);
  console.log('OK: central TEC V4/V6 — dois painéis, auditoria JSON local do Robusto, Extras sem parâmetros e responsividade validados.');
