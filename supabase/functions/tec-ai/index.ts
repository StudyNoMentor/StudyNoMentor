...
function schemaFor(section) {
  ...
  const all = { base, diagnostico: text, revisao: text, flashcards: list, quiz: list, reforco: list };
...
function policy(question, history) {
...
function promptFor(body, section) {
...
function providerError(code, upstreamStatus = 0, retryable = false, meta = {}) {
...
function classifyGeminiFailure(response, result, model) {
  const status = Number(response.status || 0);
  const upstream = String(result?.error?.status || '').toUpperCase();
  const message = String(result?.error?.message || '');
  const low = message.toLowerCase();

  if (status === 400 && /(api key not valid|invalid api key|api_key_invalid)/i.test(message)) {
    return providerError('GEMINI_API_KEY_INVALID', status, false, { model, upstream });
  }
  if (status === 401) return providerError('GEMINI_API_KEY_INVALID', status, false, { model, upstream });
...
function publicFailure(error, provider) {
  ...
  if (code === 'GEMINI_INVALID_REQUEST') {
    return { status: 424, body: { error: 'O Google rejeitou o formato da solicitação do Gemini. O servidor registrou o código técnico para correção.', code, provider, model, upstreamStatus, retryable: false } };
  }
...
async function consumeQuota(authHeader) {
...
async function callGemini(prompt, schema, signal) {
  const key = env('GEMINI_API_KEY');
  const configuredModel = env('GEMINI_MODEL') || 'gemini-3.8-flash';
  if (!key) throw providerError('PROVIDER_NOT_CONFIGURED', 0, false, { model: configuredModel });

  const models = [...new Set([configuredModel, 'gemini-3.8-flash'])];
  let lastError = null;
  ...
  for (const model of models) {
    ...
    if (!response.ok) {
      ...
      if (classified.code === 'GEMINI_MODEL_UNAVAILABLE' && model !== 'gemini-3.8-flash') continue;
      throw classified;
    }
  ...
  throw lastError || providerError('GEMINI_REJECTED', 502, false, { model: configuredModel });
}

async function callOpenAI(prompt, schema, signal) {
...
async function callProvider(provider, prompt, schema, signal) {
...
async function callProviderWithRetry(provider, prompt, schema, signal) {
  let lastError = null;
